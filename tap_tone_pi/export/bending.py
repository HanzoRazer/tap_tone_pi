...
class BendingData:
    ...
    def to_manifest_dict(self) -> Dict[str, Any]:
        ...
        if self.specific_modulus_GPa_per_gcm3 is not None:
            d["specific_modulus_GPa_per_gcm3"] = round(
                self.specific_modulus_GPa_per_gcm3, 3
            )
...
def load_bending_pair(
    ...
    if cross_grain_dir is not None:
        ...
        along_data.E_C_GPa = (
            cross_data.E_L_GPa
        )  # Cross-grain E is the "E_L" of that test
...
def validate_bending_section(bending: Dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    if "E_C_GPa" in bending and bending["E_C_GPa"] is not None:
        if not isinstance(bending["E_C_GPa"], (int, float)):
            ...
            errors.append(
                f"E_C_GPa={bending['E_C_GPa']} out of valid range (0.01-10.0)"
            )
...
def demo():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"  Specific modulus = {bending.specific_modulus_GPa_per_gcm3:.2f} GPa/(g/cm³)"
        )
