    ...
    *,
    ...
    if dominant_hz is not None and freq_uncertainty_hz is not None:
        relative_uncertainty = (
            freq_uncertainty_hz / dominant_hz if dominant_hz > 0 else 0
        )
        if (
            relative_uncertainty > th.max_relative_uncertainty
            or freq_uncertainty_hz > th.max_absolute_uncertainty_hz
        ):
            assessment.add_flag(
    ...
    if (
        dominant_hz is not None
        and reference_hz is not None
