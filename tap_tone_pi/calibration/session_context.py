...
class CalibrationContext:
    ...
    frequency_offset_hz: Optional[float] = None  # Measured frequency error (from ref tone)
...
def get_calibration_context(device_index: int) -> CalibrationContext:
    ...
    if data is None:
        ...
        return CalibrationContext(
            ...
            warnings=["Device is not calibrated. Run 'ttp calibrate' for best accuracy."],
        )
    
    # Check staleness
    is_stale = is_calibration_stale(data)
    if is_stale:
        warnings.append(f"Calibration is stale (older than 30 days). Consider recalibrating.")
