...
RULE_Q001 = RuleSpec(
    ...
    agent_actions=[
        SuggestedAction(ActionId.ADJUST_GAIN_DOWN, "Lower gain and retry", "Prevents distortion"),
...
RULE_Q002 = RuleSpec(
    ...
    agent_actions=[
        SuggestedAction(ActionId.CHECK_DEVICE, "Check input device", "Wrong input looks like silence"),
        SuggestedAction(ActionId.RETRY, "Retry with a harder tap", "Need a measurable impulse"),
        SuggestedAction(ActionId.RUN_SETUP, "Run setup wizard", "Persist the right device"),
    ],
)
...
RULE_Q003 = RuleSpec(
    ...
    agent_actions=[
        SuggestedAction(ActionId.RETRY, "Retry with better coupling", "Cleaner impulse yields clear peaks"),
        SuggestedAction(ActionId.ADJUST_DURATION_UP, "Increase duration and retry", "Gives more usable decay"),
        SuggestedAction(ActionId.HELP, "Show tapping tips", "Reduce operator variability"),
    ],
)
...
RULE_Q004 = RuleSpec(
    ...
    agent_actions=[
        ...
        SuggestedAction(ActionId.CHECK_ENVIRONMENT, "Reduce noise and retry", "Noise lowers confidence"),
...
RULE_Q005 = RuleSpec(
    ...
    agent_actions=[
        SuggestedAction(ActionId.SET_SAMPLERATE_STANDARD, "Use 48k and retry", "Policy-approved rate"),
        SuggestedAction(ActionId.RUN_SETUP, "Run setup wizard", "Persist correct rate"),
        SuggestedAction(ActionId.ABORT, "Abort", "Cannot proceed with non-standard rate"),
    ],
)
...
RULE_Q010 = RuleSpec(
    ...
    agent_actions=[
        SuggestedAction(ActionId.ACCEPT, "Accept with warning", "Measurement may still be valid"),
        SuggestedAction(ActionId.RETRY, "Retry with slightly higher level", "Improve repeatability"),
    ],
)
...
RULE_Q011 = RuleSpec(
    ...
    agent_actions=[
        SuggestedAction(ActionId.RETRY, "Retry with slightly lower gain", "Avoid clipping next attempt"),
        SuggestedAction(ActionId.ACCEPT, "Accept with warning", "Current capture is not clipped"),
    ],
)
...
RULE_Q012 = RuleSpec(
    ...
    agent_actions=[
        ...
        SuggestedAction(ActionId.ACCEPT, "Accept with warning", "If repeatable, usable"),
    ],
)
...
RULE_Q013 = RuleSpec(
    ...
    agent_actions=[
        SuggestedAction(ActionId.RETRY, "Retry with improved coupling", "May reveal clearer peak structure"),
        SuggestedAction(ActionId.ACCEPT, "Accept with warning", "Dominant peak may still be valid"),
    ],
)
...
VERDICT_FAIL = VerdictTemplate(
    ...
    default_actions=[
        ...
        SuggestedAction(ActionId.OVERRIDE, "Override (requires reason)", "Log exception explicitly", requires_input=True),
    ],
)
