...
def _run_resume(args: argparse.Namespace) -> int:
    ...
    if grid_mismatch:
        print("ERROR: Grid mismatch detected.", file=sys.stderr)
        print(f"  {grid_mismatch}", file=sys.stderr)
        print("", file=sys.stderr)
        print(
            "The session was started with a different grid configuration.",
            file=sys.stderr,
        )
        print("Options:", file=sys.stderr)
        print(f"  1. Use the original grid file: {state.grid_path}", file=sys.stderr)
        print("  2. Start a new session with --grid", file=sys.stderr)
        return 1
    ...
    print(
        f"Progress: {summary['captured'] + summary['warning']}/{summary['total']} ({summary['progress_pct']:.0f}%)"
    )
...
def _check_grid_mismatch(state: SessionState, grid: Grid) -> Optional[str]:
    ...
    if grid_order != state.point_order:
        return "Point order differs between session and grid"
...
def _do_capture(
    ...
    if synthetic:
        ...
        result = check_coherence_from_arrays(
            signal, None, fs, threshold=state.coherence_threshold
        )
...
def add_phase2_subparser(subparsers: argparse._SubParsersAction) -> None:
    ...
    run_parser.add_argument(
        "--out", type=str, default="./runs_phase2", help="Output directory"
    )
    ...
    run_parser.add_argument(
        "--duration", type=float, default=2.5, help="Capture duration (seconds)"
    )
    run_parser.add_argument(
        "--coherence-threshold", type=float, default=0.7, help="Minimum coherence"
    )
    run_parser.add_argument(
        "--synthetic", action="store_true", help="Use synthetic data (no hardware)"
    )
    run_parser.add_argument(
        "--no-progress",
        action="store_true",
