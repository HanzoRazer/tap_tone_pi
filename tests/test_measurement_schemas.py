...
def test_tap_example_validates() -> None:
    ...
    schema = json.load(open(ROOT / "contracts/schemas/tap_peaks.schema.json"))
    validate(tap, schema)


def test_moe_example_validates() -> None:
    moe = json.load(open(ROOT / "examples/measurement/bending_test.json"))
    schema = json.load(open(ROOT / "contracts/schemas/moe_result.schema.json"))
    validate(moe, schema)


def test_manifest_example_validates() -> None:
    man = json.load(open(ROOT / "examples/measurement/manifest.json"))
    schema = json.load(open(ROOT / "contracts/schemas/manifest.schema.json"))
    validate(man, schema)
