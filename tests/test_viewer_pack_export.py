...
class TestExportContract:
    ...
    def test_at_least_one_audio_point(self, exported_pack):
        ...
        _, manifest = exported_pack
        audio_files = [
            f for f in manifest["files"] if f["relpath"].startswith("audio/points/")
        ]
...
class TestKindAssignments:
    ...
    def test_audio_files_have_audio_raw_kind(self, exported_pack):
        ...
        for f in manifest["files"]:
            if f["relpath"].startswith("audio/points/") and f["relpath"].endswith(
                ".wav"
            ):
    ...
    def test_session_meta_kind(self, exported_pack):
        ...
        session_meta_files = [
            f for f in manifest["files"] if f["relpath"] == "meta/session_meta.json"
        ]
