...
class TestCheckGridMismatch:
    ...
    def test_point_count_mismatch(self, sample_grid, tmp_path):
        ...
        # Create smaller grid
        smaller_grid = Grid(
            units="mm", origin="center", points=[GridPoint(id="A1", x=0.0, y=0.0)]
        )
    ...
    def test_missing_point_ids(self, sample_grid, tmp_path):
        ...
        assert result is not None
        assert (
            "Points in session but not in grid" in result
            or "Points in grid but not in session" in result
        )
...
class TestArgumentParser:
    ...
    def test_no_progress_flag_exists(self):
        ...
        args = parser.parse_args(
            ["phase2", "run", "--grid", "test.json", "--no-progress"]
        )
    ...
    def test_coherence_threshold_configurable(self):
        ...
        args = parser.parse_args(
            ["phase2", "run", "--grid", "test.json", "--coherence-threshold", "0.8"]
        )
    ...
    def test_synthetic_flag(self):
        ...
        args = parser.parse_args(
            ["phase2", "run", "--grid", "test.json", "--synthetic"]
        )
