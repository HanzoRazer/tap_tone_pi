...
class TestCalculateMOEPlateCorrection:
    ...
    def test_strip_with_poisson_still_no_correction(self):
        ...
        result = _calculate_moe(
            **STRIP_KWARGS, poisson_ratio=0.35, specimen_type="strip"
        )
        assert result["plate_width_correction_applied"] is False

    def test_full_plate_without_poisson_no_correction(self):
        result = _calculate_moe(
            **PLATE_KWARGS, specimen_type="full_plate", poisson_ratio=None
        )
        assert result["plate_width_correction_applied"] is False
    ...
    def test_full_plate_with_poisson_applies_correction(self):
        result = _calculate_moe(
            **PLATE_KWARGS, specimen_type="full_plate", poisson_ratio=POISSON_SPRUCE
        )
        assert result["plate_width_correction_applied"] is True
        assert result["plate_width_correction_factor"] == pytest.approx(
            EXPECTED_FACTOR, rel=1e-5
        )

    def test_correction_factor_arithmetic_0_35(self):
        result = _calculate_moe(
            **PLATE_KWARGS, specimen_type="full_plate", poisson_ratio=0.35
        )
        expected = 1.0 - 0.35**2
        assert result["plate_width_correction_factor"] == pytest.approx(
            expected, rel=1e-6
    ...
    def test_correction_factor_arithmetic_0_30(self):
        result = _calculate_moe(
            **PLATE_KWARGS, specimen_type="full_plate", poisson_ratio=0.30
        )
    ...
    def test_correction_factor_arithmetic_0_40(self):
        result = _calculate_moe(
            **PLATE_KWARGS, specimen_type="full_plate", poisson_ratio=0.40
        )
    ...
    def test_new_fields_present_in_strip_output(self):
        result = _calculate_moe(**STRIP_KWARGS)
        for key in (
            "E_plate_corrected_Pa",
            "plate_width_correction_applied",
            "plate_width_correction_factor",
            "poisson_ratio_used",
        ):
...
class TestCLIWholePlate:
    ...
    def test_full_plate_with_poisson_exits_zero(self):
        proc, _ = self._run(
            "--specimen-type", "full_plate", "--poisson", "0.35", "--width", "200"
        )
        assert proc.returncode == 0, proc.stderr
