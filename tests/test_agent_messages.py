...
class TestAgentContext:
    ...
    def test_get_rule_count_with_data(self):
        ctx = AgentContext(rule_counts_this_session=(("Q001", 3), ("Q011", 1)))
    ...
    def test_get_consecutive_hits(self):
        ctx = AgentContext(consecutive_rule_hits=(("Q011", 2),))
...
class TestSortTriggeredRules:
    ...
    def test_stable_within_severity(self):
        rules = [
            TriggeredRule(rule=QualityRule("Q003", Severity.HARD, "", ""), message=""),
            TriggeredRule(rule=QualityRule("Q001", Severity.HARD, "", ""), message=""),
        ]
...
class TestUnknownRuleFallback:
    ...
    def test_unknown_rule_does_not_crash(self):
        ...
        unknown_rule = QualityRule(
            "Q999", Severity.SOFT, "Unknown rule", "Some message"
        )
        verdict = QualityVerdict(
            verdict=Verdict.WARN,
            triggered_rules=[
                TriggeredRule(rule=unknown_rule, message="Unknown condition")
            ],
        )
        ...
        assert any("Q999" in str(d) for d in msg.details) or "Q999" in str(
            msg.telemetry_tags
        )
    ...
    def test_unknown_rule_uses_rule_message(self):
        ...
        unknown_rule = QualityRule(
            "Q999", Severity.HARD, "Mystery rule", "Fallback message"
        )
        verdict = QualityVerdict(
            verdict=Verdict.FAIL,
            triggered_rules=[
                TriggeredRule(rule=unknown_rule, message="Original message")
            ],
        )
...
class TestActionSelection:
    ...
    def test_fail_includes_override_for_measure(self, fail_verdict):
        ...
        override = next(
            (a for a in msg.suggested_actions if a.action_id == "override"), None
        )
