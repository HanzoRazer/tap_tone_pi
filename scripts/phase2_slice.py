...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    base_freqs = [
        ...
        440.0,
        660.0,
    ]  # pipeline stimulus for synthetic
    ...
    for p in grid.points:
        ...
        for f, hm, coh, ph in zip(tf.freq_hz, tf.H_mag, tf.coherence, tf.H_phase_deg):
            rows.append(
                f"{float(f):.6f},{float(hm):.8e},{float(coh):.6f},{float(ph):.3f}"
            )
        save_csv(
            pdir / "spectrum.csv",
        ...
        save_json(
            pdir / "analysis.json",
            {
                ...
                "summary": {
                    "coherence_mean": float(np.mean(tf.coherence))
                    if tf.coherence.size
                    else 0.0,
                    "coherence_min": float(np.min(tf.coherence))
                    if tf.coherence.size
                    else 0.0,
    ...
    freqs, wsi, details = wsi_curve(
        spectra, grid, fmin_hz=args.fmin_hz, fmax_hz=args.fmax_hz
    )
    ...
    for f, w, d in zip(freqs, wsi, details):
        rows.append(
            f"{float(f):.6f},{float(w):.6f},{d['loc']:.6f},{d['grad']:.6f},{d['phase_disorder']:.6f},{d['coh_mean']:.6f},{d['admissible']}"
        )
    save_csv(
    ...
    def _top_points_at_freq(
        freq_bin_idx: int, max_points: int = 5
    ) -> List[Dict[str, Any]]:
    ...
    save_json(
        derived_dir / "wolf_candidates.json",
        {
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="phase2_slice",
        description="Phase 2 vertical slice: grid capture + TF/coherence + ODS snapshot + WSI",
    )
    ...
    pr.add_argument(
        ...
        type=str,
        required=True,
        help="Output root (session_* created under here)",
    )
    pr.add_argument(
        "--synthetic",
        action="store_true",
        help="Use deterministic synthetic 2ch data (no hardware required)",
    )
    pr.add_argument(
        "--device",
        type=int,
        default=None,
        help="Input device index (hardware mode only)",
    )
    ...
    pr.add_argument(
        "--top-n", type=int, default=8, help="Top N wolf candidates to write"
    )
    pr.add_argument(
        "--ods-target-hz",
        type=float,
