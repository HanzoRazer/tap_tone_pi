...
def update_references(repo_root: Path, dry_run: bool) -> List[Tuple[Path, int]]:
    ...
    replacements = [
        ...
        (r'from schemas\.measurement\.',
         'from tap_tone_pi.contracts.schemas.'),
...
def main():
    ...
    if schemas_dir.exists():
        ...
        print(f"  {'Removed' if not dry_run else 'Will remove'} {len(removed)} files/directories")
        if not dry_run:
    ...
    print(f"  • Legacy directory removed: {'Yes' if not dry_run and schemas_dir.exists() else 'N/A'}")
    ...
    if dry_run:
        ...
        print("\n✓ Migration complete!")
        print("\nNext steps:")
        print("  git add contracts/schemas/*.schema.json contracts/schema_registry.json")
        print("  git rm -rf schemas/")
        print("  git commit -m 'chore: Complete schema migration to contracts/schemas/'")
