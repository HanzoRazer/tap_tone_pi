...
def main() -> int:
    ...
    for fname in basenames:
        if fname not in filename_to_entries:
            errors.append(
                f"- {fname}: not referenced by schema_registry.json (add an entry)."
            )
            continue
    ...
    if "contracts/schema_registry.json" not in changed:
        errors.append(
            "contracts/schema_registry.json must change when contracts/schemas/* changes (version bump required)."
        )
