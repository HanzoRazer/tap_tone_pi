...
def _sanitize_peaks(peaks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for peak in peaks:
        sanitized.append({
            ...
            "coherence": round(peak.get("coherence", 0), 4) if peak.get("coherence") else None,
            "mode": peak.get("mode"),
            "q_factor": round(peak.get("q_factor", 0), 1) if peak.get("q_factor") else None
        })
    return sanitized
...
def _generate_summary(
    ...
    if coherence_stats:
        ...
        if grade in ["A", "B"]:
            ...
            summary["key_findings"].append("Measurement quality is low - consider re-measuring")
    ...
    if wood_properties:
        ...
        if grade in ["AAA", "AA"]:
            ...
            summary["key_findings"].append(f"Excellent radiation coefficient: {rad_coeff}")
        elif grade == "A":
            summary["wood_quality"] = "good"
            summary["key_findings"].append(f"Good radiation coefficient: {rad_coeff}")
        elif grade == "B":
            summary["wood_quality"] = "average"
            summary["key_findings"].append(f"Average radiation coefficient: {rad_coeff}")
        else:
            summary["wood_quality"] = "below_average"
            summary["key_findings"].append(f"Below average radiation coefficient: {rad_coeff}")
