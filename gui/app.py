...
class App(tk.Tk):
    ...
    def do_chladni_wizard(self):
        ...
        try:
            ...
            cmd_peaks = [
                sys.executable,
                "modes/chladni/peaks_from_wav.py",
                "--wav",
                wav_path,
                "--out",
                peaks_json.as_posix(),
            ...
            cmd_idx = [
                sys.executable,
                "modes/chladni/index_patterns.py",
                "--peaks-json",
                peaks_json.as_posix(),
                "--plate-id",
                plate_id,
        ...
        if binds and isinstance(binds, dict):
            ...
            for k in list(binds.keys()):
                if k.startswith(key):
                    binds[k] = var.get()
    tk.Button(
        f, text="Run", command=lambda: callback(entries if not binds else binds)
    ).pack(pady=3)
