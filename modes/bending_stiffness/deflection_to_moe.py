...
def calc(
    method, span, width, thickness, force, deflection, density=None, inner_span=None
):
    E = (
        E_3point(force, span, width, thickness, deflection)
        if method == "3point"
        else E_4point(force, span, width, thickness, deflection, inner_span)
    )
    ...
    if density is not None:
        ...
        out.update(
            {
                "specific_modulus_GPa_per_gcm3": (E / 1e9) / density,
                "c_m_s": c,
                "radiation_ratio": c / rho,
            }
        )
    return out
...
def main():
    ...
    ap.add_argument("--deflection", type=float)
    ...
    if a.csv:
        ...
        with open(a.csv, newline="", encoding="utf-8") as f:
            ...
            for row in r:
                res = calc(
                    row.get("method", "3point"),
                    float(row["span_mm"]),
                    float(row["width_mm"]),
                    float(row["thickness_mm"]),
                    float(row["force_N"]),
                    float(row["deflection_mm"]),
