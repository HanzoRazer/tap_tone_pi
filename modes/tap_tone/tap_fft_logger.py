...
def main():
    ...
    if a.plot:
        ...
        plt.xlabel("Hz")
        plt.ylabel("dB")
        plt.title("Tap Spectrum")
        for name, d in labels.items():
            plt.axvline(d["freq_hz"], color="r", alpha=0.35)
            plt.text(d["freq_hz"], plt.ylim()[1] * 0.92, name, rotation=90, va="top")
        plt.tight_layout()
        plt.savefig(a.plot, dpi=150)
